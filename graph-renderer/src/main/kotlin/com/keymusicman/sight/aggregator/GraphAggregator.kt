...
import com.keymusicman.sight.model.Connection
import com.keymusicman.sight.model.ConnectionEndpoint
import com.keymusicman.sight.model.GraphDef
import com.keymusicman.sight.model.GraphFragment
import com.keymusicman.sight.model.GraphMetadata
import com.keymusicman.sight.model.NamedGraph
import com.keymusicman.sight.model.Screen
import com.keymusicman.sight.model.Subgraph
import com.keymusicman.sight.model.filterToView
...
object GraphAggregator {
    ...
    internal data class MergedGraphDef(
        ...
        val transitions: List<com.keymusicman.sight.model.TransitionDef>,
    )
    ...
    private fun resolveFunctionTransitions(
        ...
        fragments.flatMap { it.transitions }.filter { it.source_fqn != null }.forEach { t ->
            ...
            if (sources.isEmpty()) {
                warnings += "Transition source '$source' has no matching @SightScreen — skipping"
                return@forEach
            }
            sources.forEach { src ->
                when {
    ...
    private fun resolveGraphTransitions(
        transitions: List<com.keymusicman.sight.model.TransitionDef>,
        pool: Map<Pair<String, String>, PooledScreen>,
        errors: MutableList<String>,
        warnings: MutableList<String>,
    ): List<ResolvedConn> {
        ...
        transitions.forEach { t ->
            if (t.from_screen.isBlank()) {
                errors += "Graph-level @SightTransition requires fromScreen"
                return@forEach
            }
            ...
            when {
                ...
                else -> errors += "Graph-level @SightTransition has neither toScreen nor toSubgraph"
            }
        }
        return conns.distinct()
    }
