...
class GraphSymbolProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        if (allFiles.isNotEmpty()) {
            codeGenerator.createNewFile(
                ...
                packageName = "com.keymusicman.graph.ksp",
    ...
    /** All @AppFlowScreen annotations (repeatable) on every annotated function. */
    private fun collectScreens(resolver: Resolver): List<ScreenEntry> {
        val out = mutableListOf<ScreenEntry>()
        resolver.getSymbolsWithAnnotation("com.keymusicman.graph.AppFlowScreen")
            ...
            .forEach { fn ->
                ...
                fn.annotations.filter { it.shortName.asString() == "AppFlowScreen" }.forEach { ann ->
    ...
    /** Function-level @AppFlowTransition (source_fqn = the function). */
    private fun collectFunctionTransitions(resolver: Resolver): List<TransitionEntry> {
        val out = mutableListOf<TransitionEntry>()
        resolver.getSymbolsWithAnnotation("com.keymusicman.graph.AppFlowTransition")
            .filterIsInstance<KSFunctionDeclaration>()
            .forEach { fn ->
                val fqn = fn.qualifiedName?.asString() ?: return@forEach
                fn.annotations.filter { it.shortName.asString() == "AppFlowTransition" }.forEach { ann ->
    ...
    /** Each @AppFlowGraph object + its class-level @AppFlowTransition list. */
    private fun collectGraphDefs(resolver: Resolver): List<GraphDefEntry> {
        return resolver.getSymbolsWithAnnotation("com.keymusicman.graph.AppFlowGraph")
            .filterIsInstance<KSClassDeclaration>()
            .map { cls ->
                val graphAnn = cls.annotations.first { it.shortName.asString() == "AppFlowGraph" }
                ...
                val classTransitions = cls.annotations
                    .filter { it.shortName.asString() == "AppFlowTransition" }
